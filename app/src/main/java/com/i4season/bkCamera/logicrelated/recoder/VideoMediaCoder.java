...
public class VideoMediaCoder {
    ...
    private MediaFormat F_GetMediaFormat(int i, int i2, int i3, int i4, int i5) {
        ...
        try {
            ...
            LogWD.writeMsg(this, 16, "RecoderVideoRunable   F_GetMediaFormat : " + e2.toString());
            e2.printStackTrace();
            return null;
        }
    }
    ...
    public void offerEncoder(Bitmap bitmap, int i, int i2, int i3) {
        ...
        if (iDequeueInputBuffer >= 0) {
            ...
            this.mMediaCodec.queueInputBuffer(iDequeueInputBuffer, 0, this.data.length, this.pts_, 0);
            ...
            LogWD.writeMsg(this, 16, "RecoderVideoRunable  outputBufferIndex < 0");
            return;
        }
        LogWD.writeMsg(this, 16, "RecoderVideoRunable  inputBufferIndex < 0");
        this.mMediaCodec.dequeueInputBuffer(30L);
        this.mMediaCodec.flush();
    }

    public void offerEncoder(byte[] bArr, int i, int i2, int i3) {
        MediaCodec mediaCodec = this.mMediaCodec;
        ...
        if (iDequeueInputBuffer >= 0) {
            ...
            this.mMediaCodec.queueInputBuffer(iDequeueInputBuffer, 0, bArr.length, this.pts_, 0);
            MediaCodec.BufferInfo bufferInfo = new MediaCodec.BufferInfo();
            int iDequeueOutputBuffer = this.mMediaCodec.dequeueOutputBuffer(bufferInfo, 2000L);
            if (iDequeueOutputBuffer == -2) {
                MyMediaMuxer.AddVideoTrack(this.mMediaCodec.getOutputFormat());
            ...
            if (iDequeueOutputBuffer >= 0) {
                ...
                if (bufferInfo.flags == 2) {
                    ...
                    boolean z = bufferInfo.flags == 1;
                    this.nCountFrame++;
                    MyMediaMuxer.WritSample(bArr2, z, true, this.pts_);
                }
                this.mMediaCodec.releaseOutputBuffer(iDequeueOutputBuffer, false);
            }
        }
    ...
    private byte[] getNV12(int i, int i2, Bitmap bitmap) {
        ...
        if (i3 != 39) {
            switch (i3) {
                case 19:
                    encodeYUV420P(bArr, iArr, bitmap.getWidth(), bitmap.getHeight());
                    break;
                case 20:
                    encodeYUV420PP(bArr, iArr, bitmap.getWidth(), bitmap.getHeight());
                    break;
                case 21:
            ...
            encodeYUV420PSP(bArr, iArr, bitmap.getWidth(), bitmap.getHeight());
        }
        ...
        if (this.colorFormat == 21) {
            YuvUtil.scale1422(bArr, bitmap.getWidth(), bitmap.getHeight(), bArr2, i, i2, 0);
        } else {
            YuvUtil.scale1420(bArr, bitmap.getWidth(), bitmap.getHeight(), bArr2, i, i2, 0);
        }
        return bArr2;
    }
