...
public class JudgeMultiMediaType {
    ...
    public static String getMimeType(String str) {
        MediaFileType fileType = getFileType(str);
        return fileType != null ? fileType.mimeType : UNKNOWN_STRING;
    }

    public static int getAllFileType(String str) {
        int fileTypeForMimeType = getFileTypeForMimeType(getMimeType(str));
        if (isAudioFileType(fileTypeForMimeType)) {
            return 0;
        }
        if (isVideoFileType(fileTypeForMimeType)) {
            return 1;
        }
        return isImageFileType(fileTypeForMimeType) ? 2 : -1;
    }
}
