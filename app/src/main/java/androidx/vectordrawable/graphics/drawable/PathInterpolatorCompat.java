...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        this.f38mX = new float[iMin];
        this.f39mY = new float[iMin];
        ...
        if (Math.abs(this.f38mX[0]) <= 1.0E-5d && Math.abs(this.f39mY[0]) <= 1.0E-5d) {
            int i3 = iMin - 1;
            if (Math.abs(this.f38mX[i3] - 1.0f) <= 1.0E-5d && Math.abs(this.f39mY[i3] - 1.0f) <= 1.0E-5d) {
                int i4 = 0;
                float f = 0.0f;
                while (i < iMin) {
