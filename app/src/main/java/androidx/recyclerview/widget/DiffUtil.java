    ...
    public static DiffResult calculateDiff(Callback callback, boolean z) {
        ...
        while (!arrayList2.isEmpty()) {
            ...
            if (snakeDiffPartial != null) {
                ...
                snakeDiffPartial.f30x += range.oldListStart;
                snakeDiffPartial.f31y += range.newListStart;
                Range range2 = arrayList3.isEmpty() ? new Range() : (Range) arrayList3.remove(arrayList3.size() - 1);
                range2.oldListStart = range.oldListStart;
                range2.newListStart = range.newListStart;
                if (snakeDiffPartial.reverse) {
                    range2.oldListEnd = snakeDiffPartial.f30x;
                    range2.newListEnd = snakeDiffPartial.f31y;
                } else if (snakeDiffPartial.removal) {
                    range2.oldListEnd = snakeDiffPartial.f30x - 1;
                    range2.newListEnd = snakeDiffPartial.f31y;
                } else {
                    range2.oldListEnd = snakeDiffPartial.f30x;
                    range2.newListEnd = snakeDiffPartial.f31y - 1;
                }
                arrayList2.add(range2);
                if (snakeDiffPartial.reverse) {
                    if (snakeDiffPartial.removal) {
                        range.oldListStart = snakeDiffPartial.f30x + snakeDiffPartial.size + 1;
                        range.newListStart = snakeDiffPartial.f31y + snakeDiffPartial.size;
                    } else {
                        range.oldListStart = snakeDiffPartial.f30x + snakeDiffPartial.size;
                        range.newListStart = snakeDiffPartial.f31y + snakeDiffPartial.size + 1;
                    }
                } else {
                    range.oldListStart = snakeDiffPartial.f30x + snakeDiffPartial.size;
                    range.newListStart = snakeDiffPartial.f31y + snakeDiffPartial.size;
    ...
    public static class DiffResult {
        ...
        private boolean findMatchingItem(int i, int i2, int i3, boolean z) {
            ...
            while (i3 >= 0) {
                ...
                int i7 = snake.f30x + snake.size;
                int i8 = snake.f31y + snake.size;
                if (z) {
